...
W oparciu o powyższy wykres można zauważyć następujące związki:
  ...
  * Silna dodatnia korelacja między atrybutem _ora_ (godzina) a azymutem - kąt między kierunkiem północy a kierukiem z którego świeci Słonce rośnie wraz z godziną.
  
### Wykres wytwarzania energii w zależności od czasu i lokalizacji czujnika

```{r total_fig,echo=FALSE,fig.show='animate'}
...
Powyższy wykres przedstawia sumę wyprodukowanej energii w ciągu miesiąca przez każdy z zainstalowanych paneli. Na jego podstawie można dojść do ciekawych wniosków:
  ...
  * Na wykresie z roku 2012 _panel nr 12_ wykazuje dużą produkcję energii w stosunku do reszty paneli. Jeżeli przyjrzeć się dokładniej, od sierpnia sumy wyprodukowanej energii są identyczne dla parzystych, jak i nieparzystych miesięcy. Ponieważ trudno wyjaśnić ten fakt naukowo , wygląda na to, że dane dla tego panelu zostały sfałszowane. 
  ...
  * W tym samym roku kolejne panele mają znacznie wyższy poziom produkcji energii w stosunku do reszty (uwaga ta dotyczy czterech czujników). Również w tym przypadku zachodzi podejrzenie sfałszowanej próbki.
